...
╰──────────━⃝┅⃝━────────┘`*/
 ...
 /* if (teks == '404') {
  	conn.sendHydrated(m.chat, textt, '                「 Slime BOT あ⁩ 」', global.fla + 'LIST MENU', 'https://chat.whatsapp.com/FqJndvUrrWtF4gVVvrYqMY', 'OFFICIAL GROUP', '62895412752149', 'OWNER', [
      [null,null],
      [null, null],
      [null, null]
    ], m)
 ...
 m, { contextInfo: { mentionedJid: conn.parseMention(tag) }}
    let waofc = `@${'0'.split('@')[0]}`
 m, { contextInfo: { mentionedJid: conn.parseMention(tag) }}
 let ow = `@${'62895412752149'.split('@')[0]}`
 m, { contextInfo: { mentionedJid: conn.parseMention(tag) }}
 let nurt = `@${'6281230926688'.split('@')[0]}`
 m, { contextInfo: { mentionedJid: conn.parseMention(tag) }}
 let mds = `@${'6281319777047'.split('@')[0]}`
 m, { contextInfo: { mentionedJid: conn.parseMention(tag) }}
 let mds2 = `@${'6281649478464'.split('@')[0]}`
 m, { contextInfo: { mentionedJid: conn.parseMention(tag) }}
 let admn = `@${'6281382560728'.split('@')[0]}`
 m, { contextInfo: { mentionedJid: conn.parseMention(tag) }}
  let par = `@${'6281807254833'.split('@')[0]}`
 m, { contextInfo: { mentionedJid: conn.parseMention(tag) }}
 let par2 = `@${'62818072548331'.split('@')[0]}`
 m, { contextInfo: { mentionedJid: conn.parseMention(tag) }}
